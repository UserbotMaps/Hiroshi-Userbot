...
async def logaddjoin(hiro):
    ...
    if hasattr(chat, "username") and chat.username:
        chat = f"[{chat.title}](https://t.me/{chat.username}/{hiro.action_message.id})"
    else:
        chat = f"[{chat.title}](https://t.me/c/{chat.id}/{hiro.action_message.id})"
    if hiro.user_added:
        tmp = hiro.added_by
        text = f"u📩 **#TAMBAH_LOG\n •** {vcmention(tmp)} **Menambahkan** {vcmention(user)}\n **• Ke Group** {chat}"
    elif hiro.user_joined:
        text = f"📨 **#LOG_GABUNG\n •** [{user.first_name}](tg://user?id={user.id}) **Bergabung\n • Ke Group** {chat}"
    else:
        return
    await hiro.client.send_message(BOTLOG_CHATID, text)


@bot.on(events.NewMessage(incoming=True, func=lambda e: e.is_private))
@bot.on(events.MessageEdited(incoming=True, func=lambda e: e.is_private))
async def monito_p_m_s(hiro):
    if BOTLOG_CHATID == -100:
        return
    if gvarstatus("PMLOG") and gvarstatus("PMLOG") == "false":
        return
    sender = await hiro.get_sender()
        ...
        if not no_log_pms_sql.is_approved(chat.id) and chat.id != 777000:
            if LOG_CHATS_.RECENT_USER != chat.id:
                ...
                if LOG_CHATS_.NEWPM:
                    ...
                    LOG_CHATS_.COUNT = 0
                LOG_CHATS_.NEWPM = await hiro.client.send_message(
                    BOTLOG_CHATID,
                    f"**💌 #MENERUSKAN #PESAN_BARU**\n** • Dari : **{_format.mentionuser(sender.first_name , sender.id)}\n** • User ID:** `{chat.id}`",
                )
            try:
                if hiro.message:
                    await hiro.client.forward_messages(
                        BOTLOG_CHATID, hiro.message, silent=True
                    )
                LOG_CHATS_.COUNT += 1
...
async def log(log_text):
    if BOTLOG_CHATID:
        if log_text.reply_to_msg_id:
            ...
            await edit_delete(log_text, "**Apa yang harus aing simpan?**")
            return
...
@hiro_cmd(pattern="gruplog (on|off)$")
